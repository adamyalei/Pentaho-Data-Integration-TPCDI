 ...
 * -------------------------------------------------- *
 */
--DROP FUNCTION IF EXISTS tpcdi_Audit();
--CREATE FUNCTION tpcdi_Audit()
--RETURNS VOID AS $$
--BEGIN
...
/*union
select 'DImessages validation reports', BatchID::TEXT, Result, 'Every batch must have a full set of validation reports' from (
...
*/
---ALL COMMENTED ONES HAVE A PAIR UNCOMMENTED UNDER IT WITH BATCHID = 1 TAKEN OUT. IF THIS
---BATCHID = 1 IS ADDED, THEN THE QUERY RUNS FOREVER
/*union
select 'DImessages validation reports', BatchID::TEXT, Result, 'Every batch must have a full set of validation reports' from (
	select distinct BatchID, (
		case when
	...
	from DImessages a where BatchID in (2, 3)
) o
*/
...
select 'DimBroker row count', NULL, case when
	...
	(select distinct Value from Audit where DataSet = 'DimBroker' and Attribute = 'HR_BROKERS')
...
union
select 'DimBroker BatchID', 1::TEXT, case when
...
select 'DimBroker IsCurrent', NULL, case when
	(select count(*) from DimBroker where IsCurrent::INT <> 1) = 0
...
--
union
select 'DimAccount row count', 1::TEXT, case when
	(select count(*) from DimAccount where BatchID = 1) >
	(select distinct Value from Audit where DataSet = 'DimCustomer' and Attribute = 'C_NEW' and BatchID = 1) +
	(select distinct Value from Audit where DataSet = 'DimAccount' and Attribute = 'CA_ADDACCT' and BatchID = 1) +
	(select distinct Value from Audit where DataSet = 'DimAccount' and Attribute = 'CA_CLOSEACCT' and BatchID = 1) +
	(select distinct Value from Audit where DataSet = 'DimAccount' and Attribute = 'CA_UPDACCT' and BatchID = 1) +
	(select distinct Value from Audit where DataSet = 'DimCustomer' and Attribute = 'C_UPDCUST' and BatchID = 1) +
	(select distinct Value from Audit where DataSet = 'DimCustomer' and Attribute = 'C_INACT' and BatchID = 1) -
	(select distinct Value from Audit where DataSet = 'DimCustomer' and Attribute = 'C_ID_HIST' and BatchID = 1) -
	(select distinct Value from Audit where DataSet = 'DimAccount' and Attribute = 'CA_ID_HIST' and BatchID = 1)
then 'OK' else 'Too few rows' end, 'Actual row count matches or exceeds Audit table minimum'


union
select 'DimAccount row count', BatchID::TEXT, Result, 'Actual row count matches or exceeds Audit table minimum' from (
	select distinct BatchID, (
		case when
			(select count(*) from DimAccount where BatchID = a.BatchID) >=
			(select distinct Value from Audit where DataSet = 'DimAccount' and Attribute = 'CA_ADDACCT' and BatchID = a.BatchID) +
			(select distinct Value from Audit where DataSet = 'DimAccount' and Attribute = 'CA_CLOSEACCT' and BatchID = a.BatchID) +
			(select distinct Value from Audit where DataSet = 'DimAccount' and Attribute = 'CA_UPDACCT' and BatchID = a.BatchID) -
			(select distinct Value from Audit where DataSet = 'DimAccount' and Attribute = 'CA_ID_HIST' and BatchID = a.BatchID)
...
union
select 'DimAccount EffectiveDate', BatchID::TEXT, Result, 'All records from a batch have an EffectiveDate in the batch time window' from (
	select distinct BatchID, (
		case when (
			select count(*) from DimAccount
...
select 'DimAccount IsCurrent', NULL, case when
	(select count(*) from DimAccount) =
	(select count(*) from DimAccount where EndDate = '9999-12-31' and IsCurrent::INT = 1) +
	(select count(*) from DimAccount where EndDate < '9999-12-31' and IsCurrent::INT = 0)
...
--
union
select 'DimCustomer row count', BatchID::TEXT, Result, 'Actual row count matches or exceeds Audit table minimum' from (
	select distinct BatchID, case when
		(select count(*) from DimCustomer where BatchID = a.BatchID) >=
		(select distinct Value from Audit where DataSet = 'DimCustomer' and Attribute = 'C_NEW' and BatchID = a.BatchID) +
		(select distinct Value from Audit where DataSet = 'DimCustomer' and Attribute = 'C_INACT' and BatchID = a.BatchID) +
		(select distinct Value from Audit where DataSet = 'DimCustomer' and Attribute = 'C_UPDCUST' and BatchID = a.BatchID) -
		(select distinct Value from Audit where DataSet = 'DimCustomer' and Attribute = 'C_ID_HIST' and BatchID = a.BatchID)
...
select 'DimCustomer IsCurrent', NULL, case when
	(select count(*) from DimCustomer) =
	(select count(*) from DimCustomer where EndDate = '9999-12-31' and IsCurrent::INT = 1) +
	(select count(*) from DimCustomer where EndDate < '9999-12-31' and IsCurrent::INT = 0)
then 'OK' else 'Not current' end, 'IsCurrent is 1 if EndDate is the end of time, else Iscurrent is 0'


union
select 'DimCustomer EffectiveDate', BatchID::TEXT, Result, 'All records from a batch have an EffectiveDate in the batch time window' from (
	select distinct BatchID, (
		case when (
			select count(*) from DimCustomer
			where BatchID = a.BatchID and (
...
union
select 'DimCustomer inactive customers', BatchID::TEXT, Result, 'Inactive customer count matches Audit table' from (
	select distinct BatchID, case when
		(select distinct messageData::NUMERIC from DImessages where MessageType = 'Validation' and BatchID = a.BatchID and 'DimCustomer' = MessageSource and 'Inactive customers' = MessageText) =
...
union
select 'DimCustomer age range alerts', BatchID::TEXT, Result, 'Count of age range alerts matches Audit table' from (
	select distinct BatchID, case when
		(select count(*) from DImessages where MessageType = 'Alert' and BatchID = a.BatchID and MessageText = 'DOB out of range') =
		(select distinct Value from Audit where DataSet = 'DimCustomer' and BatchID = a.BatchID and Attribute = 'C_DOB_TO') +
		(select distinct Value from Audit where DataSet = 'DimCustomer' and BatchID = a.BatchID and Attribute = 'C_DOB_TY')
	then 'OK' else 'Mismatch' end as Result
	from Audit a where BatchID in (1, 2, 3)
) o

union
select 'DimCustomer customer tier alerts', BatchID::TEXT, Result, 'Count of customer tier alerts matches Audit table' from (
	select distinct BatchID, case when
		(select count(*) from DImessages where MessageType = 'Alert' and BatchID = a.BatchID and MessageText = 'Invalid customer tier') =
		(select distinct Value from Audit where DataSet = 'DimCustomer' and BatchID = a.BatchID and Attribute = 'C_TIER_INV')
	then 'OK' else 'Mismatch' end as Result
	from Audit a where BatchID in (1, 2, 3)
) o

--I added the % to the end to allow the datatype varchar(20) to work. That way the trailing spaces will
...
select 'DimCustomer demographic fields', NULL, case when
	(
		...
		join Prospect p on upper(c.FirstName || c.LastName || c.AddressLine1 || COALESCE(c.AddressLine2,'') || c.PostalCode)
					    ...
					    and c.IsCurrent::INT = 1
	) = (
		select count(*) from DimCustomer where AgencyID is not null and IsCurrent::INT = 1
	)
...
union
select 'DimSecurity row count', BatchID::TEXT, Result, 'Actual row count matches or exceeds Audit table minimum' from (
	select distinct BatchID, case when
...
select 'DimSecurity IsCurrent', NULL, case when
	(select count(*) from DimSecurity) =
	(select count(*) from DimSecurity where EndDate = '9999-12-31' and IsCurrent::INT = 1) +
	(select count(*) from DimSecurity where EndDate < '9999-12-31' and IsCurrent::INT = 0)
then 'OK' else 'Not current' end, 'IsCurrent is 1 if EndDate is the end of time, else Iscurrent is 0'


union
select 'DimSecurity EffectiveDate', BatchID::TEXT, Result, 'All records from a batch have an EffectiveDate in the batch time window' from (
	select distinct BatchID, (
		case when (
			select count(*) from DimSecurity
			where BatchID = a.BatchID and (
...
/*
union
select 'DimCompany row count', BatchID::TEXT, Result, 'Actual row count matches or exceeds Audit table minimum' from (
	select distinct BatchID, case when
...
union
select 'DimCompany EffectiveDate', BatchID::TEXT, Result, 'All records from a batch have an EffectiveDate in the batch time window' from (
	select distinct BatchID, (
		case when (
			select count(*) from DimCompany
			where BatchID = a.BatchID and (
...
union
select 'Prospect SK_RecordDateID', BatchID::TEXT, Result, 'All records from batch have SK_RecordDateID in or after the batch time window' from (
	select distinct BatchID, (
		case when (
...
select 'Prospect MarketingNameplate', NULL, case when (
	select sum(case when (COALESCE(NetWorth,0) > 1000000 or COALESCE(Income,0) > 200000) and MarketingNameplate not like '%HighValue%' then 1 else 0 end) +
	       sum(case when (COALESCE(NumericberChildren,0) > 3 or COALESCE(NumericberCreditCards,0) > 5) and MarketingNameplate not like '%Expenses%' then 1 else 0 end) +
	       ...
	       sum(case when (COALESCE(NumericberCars,0) > 3 or COALESCE(NumericberCreditCards,0) > 7) and MarketingNameplate not like '%Spender%' then 1 else 0 end) +
...
union
select 'FactWatches row count', BatchID::TEXT, Result, 'Actual row count matches Audit table' from (
	select distinct BatchID, (
		case when
			cast((select MessageData from DImessages where MessageSource = 'FactWatches' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID order by MessageData desc limit 1) as int) -
			cast((select MessageData from DImessages where MessageSource = 'FactWatches' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID-1 order by MessageData desc limit 1) as int) =
			(select distinct Value from Audit where DataSet = 'FactWatches' and Attribute = 'WH_ACTIVE' and BatchID = a.BatchID)
		then 'OK' else 'Mismatch' end
	) as Result
	from Audit a where BatchID in (1, 2, 3)
) o
...
/*
union
select 'FactWatches active watches', BatchID::TEXT, Result, 'Actual total matches Audit table' from (
	select distinct BatchID, case when
...
/*
union
select 'FactWatches date check', BatchID::TEXT, Result, 'All SK_DateID_ values are in the correct batch time window' from (
	select distinct BatchID, (
		case when (
...
select 'FactWatches date check', BatchID::TEXT, Result, 'All SK_DateID_ values are in the correct batch time window' from (
	...
	from Audit a where BatchID in (2, 3)
) o
...
select 'DimTrade row count', BatchID::TEXT, Result, 'Actual total matches Audit table' from (
	select distinct BatchID, case when
		(select MessageData::NUMERIC from DImessages where MessageSource = 'DimTrade' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID)  =
...
--union
--select 'DimTrade date check', BatchID::TEXT, Result, 'All SK_DateID values are in the correct batch time window' from (
	--select distinct BatchID, (
	--	case when (
	...
	--				or
	--				(select DateValue from DimDate where SK_DateID = w.SK_CreateDateID) <
	--				(case when w.DT_Type like 'Limit%'  /* Limit trades can have create dates earlier than the current Batch date, but not earlier than Batch1's first date */
...
select 'DimTrade Type', NULL, case when (
	select count(*) from DimTrade
	where DT_Type not in ( 'Market Buy', 'Market Sell', 'Stop Loss', 'Limit Sell', 'Limit Buy' )
) = 0
...
select 'Financial row count', NULL, case when
		(select MessageData::NUMERIC from DImessages where MessageSource = 'Financial' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = 1)  =
...
select 'Financial FI_YEAR', NULL, case when (
	...
	(select count(*) from Financial where FI_YEAR < date_part('year', (select Date from Audit where DataSet = 'Batch' and BatchID = 1 and Attribute = 'FirstDay'))) +
	(select count(*) from Financial where FI_YEAR > date_part('year', (select Date from Audit where DataSet = 'Batch' and BatchID = 1 and Attribute = 'LastDay')))
...
union
select 'FactMarketHistory row count', BatchID::TEXT, Result, 'Actual row count matches Audit table' from (
	select distinct BatchID, (
		case when
			cast((select MessageData from DImessages where MessageSource = 'FactMarketHistory' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID order by MessageData desc limit 1) as int) -
			cast((select MessageData from DImessages where MessageSource = 'FactMarketHistory' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID-1 order by MessageData desc limit 1) as int) =
			(select distinct Value from Audit where DataSet = 'FactMarketHistory' and Attribute = 'DM_RECORDS' and BatchID = a.BatchID)
		then 'OK' else 'Mismatch' end
	) as Result
	from Audit a where BatchID in (1, 2, 3)
) o
...
/*
union
select 'FactMarketHistory SK_DateID', BatchID::TEXT, Result, 'All dates are within batch date range' from (
	select distinct BatchID, (
		case when (
			...
			(select count(*) from FactMarketHistory m where m.BatchID = a.BatchID and (select DateValue from DimDate where SK_DateID = m.SK_DateID) < (select Date - INTERVAL '1 DAY' from Audit where DataSet = 'Batch' and BatchID = a.BatchID and Attribute = 'FirstDay')) +
...
union
select 'FactHoldings row count', BatchID::TEXT, Result, 'Actual row count matches Audit table' from (
	select distinct BatchID, (
		case when
			cast((select MessageData from DImessages where MessageSource = 'FactHoldings' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID order by MessageData desc limit 1) as int) -
			cast((select MessageData from DImessages where MessageSource = 'FactHoldings' and MessageType = 'Validation' and MessageText = 'Row count' and BatchID = a.BatchID-1 order by MessageData desc limit 1) as int) =
			(select distinct Value from Audit where DataSet = 'FactHoldings' and Attribute = 'HH_RECORDS' and BatchID = a.BatchID)
		then 'OK' else 'Mismatch' end
	) as Result
	from Audit a where BatchID in (1, 2, 3)
) o
...
/*
union
select 'FactHoldings SK_DateID', BatchID::TEXT, Result, 'All dates are within batch date range' from (
	select distinct BatchID, (
		case when (
...
/*union
select 'FactCashBalances SK_DateID', BatchID::TEXT, Result, 'All dates are within batch date range' from (
	select distinct BatchID, (
		case when (
...
union
select 'Batch row count: ' || MessageSource, BatchID::TEXT,
...
union
select 'Batch joined row count: ' || MessageSource, BatchID::TEXT,
